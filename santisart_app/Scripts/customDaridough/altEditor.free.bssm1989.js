...
(function (factory) {
    ...
    $.extend(altEditor.prototype, {
        ...
        _setup: function () {
            ...
            $(this.modal_selector).bind('input', '[data-unique]', function (
                elm
            ) {
                if (
            ...
            $(`form[name="altEditor-edit-form-${this.random_id}"] *`)
                ...
                .each(function (i) {
                    if ($(this).prop('files')[0]) {
                        ++numFilesQueued;
                        that.getBase64($(this).prop('files')[0], function (
                            filecontent
                        ) {
            ...
            for (var i in dt.context[0].aoColumns) {
                ...
                columnDefs[i] = {
                    ...
                    optionsSortByLabel: obj.optionsSortByLabel
                        ...
                        : false,
                    listMenuDaridough: obj.listMenuDaridough
                        ? obj.listMenuDaridough
                        : 'text',
                    listCustomerDaridough: obj.listCustomerDaridough
                        ? obj.listCustomerDaridough
                        : 'text',
                };
            }
            return columnDefs;
        ...
        createDialog: function (
            ...
            for (var j in columnDefs) {
                ...
                if (columnDefs[j].type.indexOf('hidden') >= 0) {
                    ...
                    // Adding select-fields customers daridough
                    else if (columnDefs[j].type.indexOf('customers') >= 0) {
                        var options = '';
                        var customers = []; 
                    
                        var xmlHttp = new XMLHttpRequest();
                        xmlHttp.open("GET", "/api/DD_customer", false); // false for synchronous request
                        xmlHttp.send(null);
                        customers = JSON.parse(xmlHttp.responseText);            

                       
                        //var optionsArray = columnDefs[j].options;
                        var optionsArray = customers;
                        console.log("customer =" + optionsArray.length);
                        if (optionsArray.length > 0) {
                            // array-style select or select2
                            for (var i = 0; i < optionsArray.length; i++) {
                                options +=
                                    "<option value='" +
                                    this._quoteattr(optionsArray[i].index) +
                                    "'>" +
                                    optionsArray[i].Recipient_s_name +
                                    '</option>';
                            }
                        } else {
                            // object-style select or select2
            ...
            for (var j in columnDefs) {
                if (columnDefs[j].select2) {
                    ...
                    console.log("select2 jquery");
                    var replaceSelectDot = columnDefs[j].name.replace(/[!"#$%&'()*+,.\/:;<=>?@[\\\]^`{|}~]/g, "\\$&")
                    $(selector)
                        .find('select#' + replaceSelectDot)
                        .select2({ dropdownParent: $(selector), width: "100%" });
